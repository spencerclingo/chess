...
public class TestFactoryServer {
        ...
        for (var c : boardText.toCharArray()) {
            switch (c) {
                ...
                default -> {
                    ...
                    var position = TestFactoryServer.getNewPosition(row, column);
                    var piece = TestFactoryServer.getNewPiece(color, type);
    ...
    public static Set<ChessMove> loadMoves(ChessPosition startPosition, int[][] endPositions) {
        ...
        for (var endPosition : endPositions) {
            validMoves.add(TestFactoryServer.getNewMove(startPosition,
                    TestFactoryServer.getNewPosition(endPosition[0], endPosition[1]), null));
        }
        return validMoves;
    }
}